...
def main():
    ...
    training_records = []
    reword_single = []
    if os.path.isfile('log/dqn_reward.np'):
    ...
    with open('log/dqn_reward.np', 'wb') as f:
        np.save(f,result)
...
if __name__ == '__main__':
    if os.path.isfile('log/1.np'):
        reward_records = np.load('log/1.np', allow_pickle=True)
        result = reward_records / np.linalg.norm(reward_records, axis=-1)[:, np.newaxis]
        print(result)
        var = np.var(reward_records, axis=1)
        print(var)
